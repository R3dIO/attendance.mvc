...
if(is_uploaded_file($_FILES['file']['tmp_name']))
{
			...
			if($result) {
				...
				if($res) {
					$r = mysqli_query($conn,"INSERT IGNORE INTO attendance_table (schedule_id,student_id) SELECT h.id,s.id FROM student_table s,schedule_table h,class_table c WHERE c.id=h.class_id AND h.class_id=s.class_id AND h.batch=s.batch;");
					if($r)
	...
	if($msg == '')
		...
		$rs = mysqli_query($conn,"select distinct token from firebase_tokens t,faculty_subject_table s where t.faculty_id=s.faculty_id;");
		$tokens = array();
